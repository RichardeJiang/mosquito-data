	...
	# for bgVideo in bgVideoList:
		...
		for i in range(trackNum):
			...
			for frameCounter, frameID in enumerate(currentTrackFrames):
				...
				dst = cv2.resize(mask, None, fx = 1/downScale, fy = 1/downScale)
				dstImg = cv2.resize(img, None, fx = 1/downScale, fy = 1/downScale)
				...
				# originally, added this check logic so as to add in manual -1 cases; now we have moving out of window
				# so we can remove this later
